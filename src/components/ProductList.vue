...
const getProducts = async ({
  ...
  try {
    ...
    console.log('Products loaded:', response.data.result.length)
...
<template>
  <v-infinite-scroll v-if="isListVisible" @load="getProducts">
    <template v-for="product in products" :key="product.id">
      <v-card variant="flat" elevation="1" shadow color="grey-lighten-5
        ...
        <v-row>
          ...
          <v-col cols="8" class="d-flex flex-column">
            ...
            <div class="mt-2 flex-grow-1 d-flex justify-space-between align-end">
              ...
              <v-btn :text="$t('shared.more')" variant="plain" class="px-0" append-icon="chevron_left" density="compact"
                :to="{
